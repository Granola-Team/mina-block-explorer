...
RSpec.describe "Internal Commands table", type: :system do
    ...
    test_filter(
      ...
      lambda do
        ...
        recipient_cells.each do |cell|
          cleaned_text = cell.text.gsub(/[\n+-]/, "")
          expect(cleaned_text).to include("B62qioAD9geuKsffk9gXSgHf18riNEB9NmR4Zyuo2fvWd5WWYTg4WHB"), "Expected 'Recipient' to be 'B62qioAD9geuKsffk9gXSgHf18riNEB9NmR4Zyuo2fvWd5WWYTg4WHB', but was '#{cleaned_text}'"
        end
      end
    )
  end
  ...
  it "has standard row limits" do
    ...
    [25, 50, 100, 250, 500, 1000].each { |limit|
      ...
      table_rows = get_table_rows("Internal Commands")
      expect(table_rows.count).to be == limit, "Expected 'Internal Commands' table to have row count of #{limit}, but found #{table_rows.count}"
    }
  end
