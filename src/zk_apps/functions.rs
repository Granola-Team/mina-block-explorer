...
pub fn stub_zk_apps_data(size: u64) -> Vec<Option<ZkAppData>> {
    let mut rng = rand::thread_rng();
    (0..size)
        .map(|_| {
            Some(ZkAppData {
                _validator_name: generate_random_string(10),
...
pub fn stub_zk_app_txn_data(size: u64) -> Vec<Option<ZkAppTransactionData>> {
    let mut rng = rand::thread_rng();
    (0..size)
        .map(|_| {
