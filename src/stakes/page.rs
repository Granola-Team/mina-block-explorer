    ...
    match epoch {
        ...
        Some(epoch) => {
            let query_body = format!(
                r#"{{"query":"query StakingLedgersQuery($limit: Int = 1, $query: StakeQueryInput!) {{stakes(limit: $limit, query: $query) {{ epoch epoch_num_accounts  }}}}", "variables":{{"epoch":"{}", "limit":{}}},"operationName":"StakingLedgersQuery"}}"#,
                epoch, 1
            );
                ...
                view! {
                    <StakesPageContents
                        ...
                        epoch_num_accounts=resource
                            .get()
                            .and_then(|res| res.ok())
                            .and_then(|s| s.data.first().cloned())
