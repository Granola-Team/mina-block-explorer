...
suite(["@VisualRegression"], "spotlight", () => {
  const pages = [
    {
      url: "/summary/accounts/B62qoiQhNWjwFkfCVBpkDTYytifftoYQB9qJ3z6X4d58ocxmiwUQY8U",
      name: "account-dialog",
    },
  ...
  pages.forEach((page) => {
    devices.forEach((device) => {
      it(`has elements correctly positioned on page ${page.name} on ${device}`, () => {
        ...
        cy.get("#spotlight-heading").matchImageSnapshot(
          `[${page.name}]-[${device}]-spotlight-heading`,
        );
      });
    });
  });
