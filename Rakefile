...
MARKDOWN_SRC_FILES = Dir.glob("**/*.md").reject { |file| file.start_with?("lib", "rust/target", "ruby/vendor/") }
GRAPHQL_SRC_FILES = Dir.glob("rust/graphql/**/*.graphql")
...
RUBY_SRC_FILES = Dir.glob("**/*.rb").reject { |file| file.start_with?("lib/", "ruby/vendor/") } + ["Rakefile"]
